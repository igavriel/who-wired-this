...
| [static-scoreboard-refactor.md](static-scoreboard-refactor.md) | **Implemented:** Static `ScoreManager` run scoreboard (per-player retries/time per level); fold `PlaytestRunTotal`; delete legacy numeric score; GameOver per-level summary. |
| [gameover-50x12-summary-grid.md](gameover-50x12-summary-grid.md) | **Implemented:** Fixed 50×12 monospace Game Over run summary grid so status + Blue/Red per-level retries/times fit without clipping. |
| [gameover-idle-return-timer.md](gameover-idle-return-timer.md) | **Implemented:** 30s idle timeout on GameOver; if Restart/CTRL (OK) not pressed, auto-return to StartScene. |
| [team-score-0-100.md](team-score-0-100.md) | **Implemented:** Combined 0–100 team score (time 2/5/8 bands + attempts −2 each); drop best-time/rank; 8:00 per-level TopBar countdown. |
| [diagnostic-panel-operation-guide-40x12.md](diagnostic-panel-operation-guide-40x12.md) | **Implemented:** Formatter-driven 40×12 operator/reader guides; DiagnosticPanel + Tutorial SceneStageManager wiring. |
...
| [startscene-trailer-intro-sequence.md](startscene-trailer-intro-sequence.md) | **Implemented:** 30s name+instructions; hide YoutubeAnchor until play; loop trailer 56s then restore UI. |
